...
function loadReview() {
  ...
  // Set first review as active
  if (reviews.length > 0) {
    reviews[0].classList.add("active");
  }
  
  dotReview.forEach(function (dot, dotIndex) {
  ...
  function slide_rv(newIndex) {
    // Remove active from all reviews
    reviews.forEach(rv => rv.classList.remove("active"));
    dotReview.forEach(dot => dot.classList.remove("active"));
    
    // Add active to current review
    reviews[newIndex].classList.add("active");
    dotReview[newIndex].classList.add("active");
    index_rv = newIndex;
  }
