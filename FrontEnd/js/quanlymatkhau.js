// Kiểm tra quyền truy cập - chỉ cho người dùng đã đăng nhập
(function() {
  ...
  function checkAccess() {
    ...
    if (window.roleCheck && !window.roleCheck.isLoggedIn()) {
      console.warn('Người dùng chưa đăng nhập, chuyển hướng tới trang đăng nhập');
      window.location.href = './dangnhap.html';
      return;
    }

    // Thực hiện kiểm tra lại sau 1 giây để đảm bảo authSync đã load
    setTimeout(() => {
      if (window.roleCheck && !window.roleCheck.isLoggedIn()) {
        window.location.href = './dangnhap.html';
      }
